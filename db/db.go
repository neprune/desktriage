...
// Pragmas are passed via the DSN so every pooled connection gets them.
func Open(path string) (*sql.DB, error) {
	// Use DSN pragma parameters so every connection in the pool inherits them.
	// Executing PRAGMAs via db.Exec only affects the one checked-out connection,
	// leaving other pooled connections without busy_timeout — causing SQLITE_BUSY
